...
async function canonicalIntent({ capabilityId = "code.test", operation = "test", subject = "damar" } = {}) {
    ...
    if (!canonicalBound) {
        ...
        await registry.proposeEvolution({
            ...
            requestedAuthority: { capabilityId, subject, actions: [operation], scope: ["."], maxExecutions: 500 }
        }, "owner");
...
function rig() {
 ...
 // R2-02: no authorityBridge parameter exists — the canonical source is
 // resolved live at route time (bound above).
...
test("L3: route leases to eligible trusted node; lease binds intent+node+generation+nonce", async () => {
 ...
 const out = await router.route({ intent, toolId: "code_test" });
...
test("L3: untrusted node never routed — trust scope beats score", async () => {
 ...
 const intent = await canonicalIntent();
 await assert.rejects(() => router.route({ intent, toolId: "code_test" }), (e) => e.code === "NODE_UNTRUSTED");
...
test("L3: revoked node rejected mid-routing; capability disappearance removes route", async () => {
 ...
 const intent = await canonicalIntent();
 const out = await router.route({ intent, toolId: "code_test", preferredNodeId: remoteId });
 ...
 await assert.rejects(() => router.route({ intent, toolId: "code_test", localPreferred: false }), (e) => e.code === "ROUTE_UNAVAILABLE" || e.code === "NODE_UNTRUSTED");
 ...
 await assert.rejects(() => router.route({ intent, toolId: "code_test" }), (e) => e.code === "NODE_UNTRUSTED" || e.code === "ROUTE_UNAVAILABLE");
});
...
test("L3: privacy placement — SECRET_REFERENCE input never routes to REMOTE_COMPUTE", async () => {
 ...
 const intP = await canonicalIntent();
 const localOnly = await router.route({ intent: intP, toolId: "code_test", privacyClass: "SECRET_REFERENCE" });
 ...
 const pub = await router.route({ intent: intP, toolId: "code_test", privacyClass: "PUBLIC", preferredNodeId: remoteId });
...
test("L3: remote result verification — forged result digest rejected; legal states only", async () => {
 ...
 const intent2 = await canonicalIntent();
 const out2 = await router.route({ intent: { ...intent2, intentId: "intent-2" }, toolId: "code_test", preferredNodeId: remoteId });
