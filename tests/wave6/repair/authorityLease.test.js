...
/**
 * W6-02 / R2-02 — canonical authority provenance + W6-03 lease consumption.
 * Authority is resolved via LIVE evaluation through the module-private
 * canonical source (bound exactly once to the canonical AuthorityRegistry).
 * There is no caller-supplied digest, no caller-supplied bridge, no
...
test("W6-02: router cannot be constructed before canonical authority is bound — but bind is first-wins", () => {
 // already bound by prior test (module singleton); verify bind is idempotent
 const store = createMemoryAuthorityStore();
 const registry = new AuthorityRegistry({ store, clock: { nowIso: () => new Date(1_000_000).toISOString() } });
 assert.throws(() => bindCanonicalAuthorityRegistry(registry), /already bound/);
 // brand check
 assert.ok(isCanonicalAuthorityRegistry(registry));
});
