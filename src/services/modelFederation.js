...
class EntityModelFederation {
    constructor({ providers, wises } = {}) { if (!providers || !wises) throw new TypeError("FEDERATION_DEPENDENCIES_REQUIRED"); this.providers = providers; this.wises = wises; this.assignments = new Map(); }
    ...
    describe(entityId) { const p = this.assignments.get(entity(entityId)); return p ? Object.freeze({ entityId: p.entityId, primaryRoute: p.primaryRoute && { ...p.primaryRoute }, configuredFallbacks: p.configuredFallbacks.map(x => ({ ...x })), routingMode: p.routingMode, policy: { ...p.policy }, systemFallback: "wises-d1" }) : null; }
