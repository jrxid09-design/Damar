...
//
// 1. Every issuance carries a bounded validity: generation (opaque epoch
// identity at creation) + expiresAt (creation + DELEGATION_TTL_MS). A
// delegation outside its validity is rejected as STALE/EXPIRED — never
// accepted merely because its tombstone happens to be absent.
...
// rotated: every outstanding issuance of the previous epoch is
...
//
// RA3-02 safe epoch identity: the epoch is an OPAQUE 128-bit random
// identifier (pdlep_<32 hex>), NOT a monotonically incremented JS Number.
// A numeric `epoch++` loses precision beyond Number.MAX_SAFE_INTEGER — at
// that boundary an old issuance generation could compare equal to the
// rotated epoch and reopen replay. Exact string identity (`!==`) has no
// precision semantics: old-epoch issuances fail because the epoch
// identity DIFFERS, no matter how many rotations occurred. The identity
...
function createDelegation({ fromEntity, toEntity, sourceSession, targetSession = null, objective, reason, contextRefs = [], requestedCapabilities = [] } = {}) {
 ...
 // RA3-02: issuance is bound to the immutable opaque epoch identity.
 // Exact-safe comparison only (string identity, no numeric precision).
 generation: epochId,
 state: "ISSUED"
 });
...
function acceptDelegation(delegation, { receiver, sessionId, now = Date.now() } = {}) {
 ...
 // RA3-02: exact-safe opaque epoch identity check. An old-epoch issuance
 // fails because its generation identity DIFFERS from the current epoch —
 // never because of numeric precision. Caller-supplied/forged generation
 // values cannot match unless they were minted by this module.
 if (delegation.generation !== epochId) throw new TypeError("PANDAWA_DELEGATION_STALE");
 ...
 // outstanding issuance of the previous epoch becomes STALE
 // (fail-closed), which is what makes evicting their tombstones safe.
 // Bounded, deterministic, crash-free — no counter mutation, no
